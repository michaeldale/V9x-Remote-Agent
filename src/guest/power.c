...
static int v9x_power_error(V9xConnection *conn, DWORD request_id,
                             ...
                             unsigned long length)
{
    ...
    UINT flags;
    if (v9x_execution_any_active(machine)) {
        return v9x_power_error(conn, request_id, V9X_STATUS_BUSY, 0ul,
                               "execution active");
    }
    if (!v9x_read_job(payload, length, job)) {
        return v9x_power_error(conn, request_id, V9X_STATUS_INVALID_PAYLOAD,
                               ERROR_INVALID_PARAMETER, "invalid resume token");
    }
    if (!v9x_persist_job(job, &native_error)) {
        return v9x_power_error(conn, request_id, V9X_STATUS_IO_FAILED,
                               native_error, "resume token persistence failed");
    }
    {
        unsigned long index = 0ul;
        do {
            machine->pending_job[index] = job[index];
        } while (job[index++] != '\0' && index < sizeof(machine->pending_job));
    }
    v9x_files_disconnect(conn);
    v9x_write_u32(conn->power_response, machine->boot_counter);
    if (!v9x_append_string(conn->power_response, sizeof(conn->power_response),
                           &offset, job)) return 0;
    if (type == V9X_MSG_REBOOT_REQUEST) {
        response_type = V9X_MSG_REBOOT_ACCEPTED;
        flags = EWX_REBOOT | EWX_FORCE;
        v9x_log_event("reboot-accepted", job);
    } else {
        response_type = V9X_MSG_SHUTDOWN_ACCEPTED;
        flags = EWX_SHUTDOWN | EWX_POWEROFF | EWX_FORCE;
        v9x_log_event("shutdown-accepted", job);
    }
    if (!v9x_send_frame(conn, response_type, request_id,
                        conn->power_response, offset)) return 0;
    Sleep(250ul);
